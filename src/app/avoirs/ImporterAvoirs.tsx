...
import { importerAvoirsExcelAction } from "../actions";
import type { ImportResultat } from "@/lib/registre";

export default function ImporterAvoirs() {
  ...
  function importer() {
    ...
    startTransition(async () => {
      const res = await importerAvoirsExcelAction(formData);
      if (!res.ok) {
  ...
  if (!ouvert) {
    return (
      <button
        ...
        className="rounded border border-amber-300 px-4 py-2 text-sm font-semibold text-amber-700 hover:bg-amber-50"
      >
