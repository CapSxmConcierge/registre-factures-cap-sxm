...
import { ajouterFactureAction } from "../actions";
...
export default function AjouterFacture() {
  ...
  const [venteMateriel, setVenteMateriel] = useState(false);
  const [concerneTgca, setConcerneTgca] = useState(true);
  ...
  function reinitialiser() {
    ...
    setMontantTtc("");
    setMontantHt("");
    setVenteMateriel(false);
    setConcerneTgca(true);
    setErreur(null);
  ...
  function soumettre() {
    ...
    if (venteMateriel) formData.set("venteMateriel", "on");
    ...
    startTransition(async () => {
      const resultat = await ajouterFactureAction(formData);
      if (!resultat.ok) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      <p className="text-sm font-semibold text-slate-800">Nouvelle facture (numéro attribué automatiquement)</p>
      ...
      <div className="mt-3 space-y-2">
        <label className="flex items-center gap-2 text-sm text-slate-700" title="OUI uniquement pour une vente de matériel">
          <input type="checkbox" checked={venteMateriel} onChange={(e) => setVenteMateriel(e.target.checked)} />
          Vente de matériel
          <span className="text-xs text-slate-400">
            (OUI seulement si c&apos;est une vente de matériel — NON pour une prestation diverse, un remboursement de
            frais ou une facture de séjour)
          </span>
        </label>
        <label className="flex items-center gap-2 text-sm text-slate-700">
          ...
          <span className="text-xs text-slate-400">
            (OUI pour vente de matériel/prestations diverses/gestion décompte — NON pour un remboursement de frais ou
